...
import { SquareCard } from "../../../core/src";
import { LaravelBackendConfig } from "../../src/config/laravelBackendConfig";

export class LaravelBackendImportSquareCards implements BaseCommand {
    static signature: string = "laravel-backend:import-square-cards";

    static description: string = "Imports Square cards data from Laravel " +
        "backend and attaches the same to it's respective patients and " +
        "Square customers.";
    ...
    async handle(options: any, command: any) {
        console.info(chalk.cyan("Importing Square cards"));

        const schemaName = "SquareCard";
        ...
        while (true) {
            ...
            try {
                ...
                const totalRecordResult = await mysqlPool.query("SELECT COUNT(*) as totalRecords from square_cards");
                ...
                while (true) {
                    ...
                    const dataQuery = `SELECT square_cards.*, patients.uid as patientUid ` +
                        `FROM square_cards ` +
                        `LEFT JOIN square_customers ON square_cards.customer_id = square_customers.uid ` +
                        `LEFT JOIN patients ON square_customers.uid = patients.square_customer_uid ` +
                        (!lastId ? '' : `WHERE square_cards.id > ${lastId} `) +
                        `ORDER BY square_cards.id ` +
                        `LIMIT ${pageSize}`
                    ...
                    for await (const row of dataRows) {
                        ...
                        let object = await new Parse.Query(SquareCard)
                        ...
                        if (!isExisting) {
                            object = new SquareCard();
                            object.id = row.uid;;
                        }

                        // Populate data
                        object.patient = row.given_name;
                        object.billingAddress = row.billing_address;
                        object.fingerprint = row.fingerprint;
                        object.bin = row.bin;
                        object.cardBrand = row.card_brand;
                        object.cardType = row.card_type;
                        object.cardholderName = row.cardholder_name;
                        object.enabled = row.enabled === 1;
                        object.expMonth = parseInt(row.exp_month);
                        object.expYear = parseInt(row.exp_year);
                        object.last4 = row.last_4;
                        object.merchantId = row.merchant_uid;
                        object.prepaidType = row.prepaid_type;
                        object.referenceId = row.reference_id;
                        object.version = parseInt(row.version);
                        ...
                        // Attach Square customer
                        const squareCustomer = new SquareCustomer();
                        squareCustomer.id = row.customer_id;
                        object.squareCustomer = squareCustomer;
...
// Command: thdc laravel-backend:import-square-card